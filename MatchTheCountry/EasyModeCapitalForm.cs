...
{
    public partial class EasyModeCapitalForm : Form
...

    {
        ...
        void updateFlagCapital()
        {
            ...
            {// if all of the indices have been used, clear the indices array to start over
                if (previouslyUsedIndicesCapital.Count == EasyMode.capital.Length)
                    previouslyUsedIndicesCapital.Clear();

                // generate a random index between 0 and the number of names
                randomIndexCapital = random.Next(0, EasyMode.capital.Length);
            }
            ...
            flagPictureBoxCapital.Image = EasyMode.flag[randomIndexCapital];

            // Put the name of the country into the country name label
            countryNameLabel.Text = EasyMode.name[randomIndexCapital];
        ...
        void CheckAnswer()
        {
            ...
            string capitalCityName = EasyMode.capital[randomIndexCapital];
            ...
            { // Show a message telling the user they are not correct
                capitalWindowLabel.Text = $"Incorrect- The correct answer was {capitalCityName}";
        ...
        private void capitalTimer_Tick(object sender, EventArgs e)
        {
